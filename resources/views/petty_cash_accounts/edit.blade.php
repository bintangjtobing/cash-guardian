@extends('welcome')
@section('title', 'Edit Petty Cash Account')
@section('content')
    <div class="col-lg-8">
        <div class="card card-Vertical card-default card-md mb-4">
            <div class="card-body pb-md-30">
                <form action="{{ route('petty-cash-accounts.update', $account->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="card-header">
                            <h6>Edit Petty Cash Account</h6>
                        </div>
                        ...
                        <div class="col-md-12 mb-25">
                            <label for="group_id" class="color-dark fs-14 fw-500 align-center">Group</label>
                            <select name="group_id" id="group_id"
                                class="form-control ih-medium ip-gray radius-xs b-light px-15" required>
                                <option value="" disabled>Select Group</option>
                                @foreach ($groups as $group)
                                    <option value="{{ $group->id }}"
                                        {{ $group->id == $account->group_id ? 'selected' : '' }}>
                                        {{ $group->name }}</option>
                                @endforeach
                            </select>
                        </div>
